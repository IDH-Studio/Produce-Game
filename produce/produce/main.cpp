...
void Init(SELECT* select, Factory factory[], Player** player, Store** store, AREA* area, FiberFarm fiberFarm[], FabricFactory fabricFactory[])
{
	setCursor(false);
...
void Update(SELECT* select, Factory* factory, Player* player, Store* store, AREA* area, FiberFarm* fiberFarm, FabricFactory* fabricFactory)
{
	int key = 0;
...
void Render(SELECT select, Factory* factory, Player player, Store* store, AREA area, FiberFarm* fiberFarm, FabricFactory* fabricFactory)
{
	clearBuffer();
...
int main(int argc, char* argv[])
{
	...
	Init(&select, factory, &player, &store, &area, fiberFarm, fabricFactory);

	while (gameRun)
	{
		Update(&select, factory, player, store, &area, fiberFarm, fabricFactory);
		Render(select, factory, *player, store, area, fiberFarm, fabricFactory);
		Sleep(33);
	}
